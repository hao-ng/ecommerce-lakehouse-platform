...
from common.config import CLICKSTREAM
...
def execute(spark, config, schema_registry_client):
...
def main():
    spark = get_spark_session("Clickstream Ingestion")
    schema_registry_client = SchemaRegistryClient(SCHEMA_REGISTRY_URL)

    query = execute(spark, CLICKSTREAM, schema_registry_client)
