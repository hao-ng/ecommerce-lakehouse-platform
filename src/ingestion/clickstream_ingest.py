...
from common.config import BRONZE_APPEND_TABLES, BronzeConfig
...
def execute(
    ...
    logger.info("Starting clickstream ingestion job")
...
def main():
    spark = get_spark_session("Clickstream Ingestion", env)
    schema_registry_client = SchemaRegistryClient(env.schema_registry_url)

    config = BRONZE_APPEND_TABLES["clickstream"]
    query = execute(spark, env, config, schema_registry_client)
